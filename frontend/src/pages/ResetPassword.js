...
const ResetPassword = () => {
  ...
  const handleSubmit = async(payload) => {
    if(payload.password === payload.rePassword) {
        ...
        console.log(res);
        if(res?.success) {
            Swal.fire({title: "Thay đổi mật khẩu thành công!", icon: 'success', timer: 1500, text: 'Vui lòng đăng nhập lại!'}).then(() => {
                navigate('/')
            })
        }
        else {
        ...
        Swal.fire({title: 'Thất bại!', text: "Nhập lại mật khẩu không chính xác!", icon: 'info', timer: '1500'})
    }
  };
  return (
